...
export const ExportPanel = memo(function ExportPanel() {
  ...
  const output = useMemo(() => (isOpen ? exportJson() : ""), [exportJson, isOpen, project]);

  return (
    <>
      <button className={styles.exportButton} type="button" onClick={() => setIsOpen(true)}>
        <FileDownloadOutlinedIcon aria-hidden="true" />
        Export
      </button>

      <Modal open={isOpen} onClose={() => setIsOpen(false)} size="lg" closeOnBackdrop={false}>
