...
2. В библиотеке создайте проект: выберите разрешение дисплея, ориентацию и шаблон. Доступны встроенные шаблоны Blank и Hello, а также пользовательские — любой проект можно назначить шаблоном через **Project → Use as template**. Создание из пользовательского шаблона копирует его содержимое на выбранный размер дисплея; дальнейшие правки исходного шаблона уже созданные проекты не меняют.
